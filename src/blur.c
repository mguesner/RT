...
static void		sweet(int px, int py, t_libx *m, char *save)
{
	...
	inc = (m->pix[pospix]->dist - m->ref_dist_mblur) * 8 / (m->ref_maxdist_mblur);
	while (inc > 0)
	{                                          
...
static char		*copy_to(t_libx *m)
{
	...
	while (px < 1080 * 1920)
	{
		if (m->pix[px]->dist)
		{
			...
			if (m->pix[px]->dist > m->ref_maxdist_mblur)
				m->ref_maxdist_mblur = m->pix[px]->dist;
		}
		px += 1;
	}
	return (save);
